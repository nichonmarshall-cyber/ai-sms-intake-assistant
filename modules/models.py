...
class AppointmentRequest(Base):
    """A tenant-owned scheduling request that may become a provider event."""

    __tablename__ = "appointment_requests"
    __table_args__ = (
        UniqueConstraint("business_id", "lead_id", name="uq_appointment_requests_business_lead"),
        Index("ix_appointment_requests_business_status", "business_id", "status"),
        Index("ix_appointment_requests_business_start", "business_id", "scheduled_start_at"),
    )
    ...
    service_request: Mapped[str | None] = mapped_column(String(500), nullable=True)
    requested_time_text: Mapped[str | None] = mapped_column(String(160), nullable=True)
    scheduled_start_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    duration_minutes: Mapped[int] = mapped_column(Integer, nullable=False, default=60)
    # pending | scheduled | declined | cancelled | sync_failed
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="pending")
    calendar_event_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    calendar_event_link: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    provider_error: Mapped[str | None] = mapped_column(String(255), nullable=True)
    scheduled_by_user_id: Mapped[str | None] = mapped_column(
        ForeignKey("platform_users.id", ondelete="SET NULL"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
